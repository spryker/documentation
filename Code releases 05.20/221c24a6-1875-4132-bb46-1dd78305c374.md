...
## Claiming Orders
By claiming an order, you create a [reclamation](https://documentation.spryker.com/docs/reclamations-feature-overview-201903). A claimed order is flagged as "Reclamation". The reclamations are not linked to neither an order status nor a state machine. It's because this functionality is different in every project. Also, order claims are checked on an individual basis and may not influence order item status at all.

**To create a [reclamation](https://documentation.spryker.com/docs/reclamations-feature-overview-201903) for an order:**
