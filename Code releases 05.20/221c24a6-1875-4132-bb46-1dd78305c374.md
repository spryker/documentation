When a customer buys something from your online store, the information on their order is automatically transferred to the Orders page. This page is designed to manage all orders created by your customers.
To start managing an order, go to **Sales > Orders** section and click **View** in the _Actions_ column for a specific order.
...
* updating the internal custom order reference
***
## Setting Order Statuses 

**To set statuses for all items in the orders:**
1. In the **View Order: [Order ID] > Trigger all matching states** section, click **pay**, which is the first status in the flow.
...
To learn more about the order statuses, see [Orders: Reference Information](https://documentation.spryker.com/v4/docs/orders-reference-information).

## Editing Custom Order Reference
**To edit a custom order reference:**

1. In the **View Order: [Order ID] > Custom Order Reference** section, click **Edit Reference**.
...
2. Update the custom order reference and click **Save**.

**Tips & Tricks**

If you want to discard the changes, click **Cancel**.

If you want to remove the custom order reference, clear the **Custom Order Reference** field and click **Save**.
