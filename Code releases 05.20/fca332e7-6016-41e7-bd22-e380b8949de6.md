...
1. Open a terminal.
@(Info)()(In case you are going to run the script on Windows, open Ubuntu (For more details, see the *Install Docker Desktop* section in [Docker Installation Prerequisites - Windows](https://documentation.spryker.com/docs/docker-installation-prerequisites-windows).))
...
:::
5. Because default spryker php image has been upgraded, please edit `deploy.yml` file on 7th line to use specific image that is working with this release of the demoshop:
from
```shell
...
```
:::
7. Bootstrap local docker setup for demo:
```shell
