...
2. Specify necessary configurations in the .yml export configuration file residing in `./data/export/config/`. See .yml Export Configuration File for details on the .yml export config file structure and configuration options.
3. Run `console data:export --config file-name.yml`, where `file-name.yml` is the name of the .yml export configuration file. After successful run of this command, export .csv files will be created in `./data/export/`folder for each *data_entity* of the .yml file. For each store specified in the .yml file, a separate file is created. See Structure of the .yml Export Configuration File for example of how the export works.
...
By default, a .yml export configuration file resides in `./data/export/config/`.  You can adjust your .yml export configuration file, but when doing so, stick to its structure and take the possible data filtering options into account.
...
actions:  
   ...
   -  data_entity: order-expense             
      ...
      filter_criteria:
          ...
          store_name: [<store_name_value_1>]
```
Type of content to export is defined in section *actions* by *data_entity* and must be *order*, *order-item* and *order-expense* . You can define for what stores you want to run export, as well as specify order dates you want to export data for. See Setting the Filter Criteria in a .yml File for details on how to export order data for specific stores and time period.
