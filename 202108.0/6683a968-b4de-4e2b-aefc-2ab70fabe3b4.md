...
@(Info)(Example)(For instance, these could be separate shopping lists for daily, weekly, and monthly purchases.)
...
@(Info)()(Alternative products are not shown for discontinued products since they can not be basically added to the cart, and a user with *Read only* rights can not amend the shopping list by adding these products to it.)
...
@(Info)()(Besides other products, a user with *Full access* rights is allowed to see alternatives for discontinued items, which can be added to the shopping list.)
