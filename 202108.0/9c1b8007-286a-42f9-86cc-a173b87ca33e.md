...
- Send the `GET http://glue.mysprykershop.com/companies/{{company_uuid}}` request. Make sure that the response contains only the company resource your current company user belongs to.
...
- Send the `GET http://glue.mysprykershop.com/company-business-units/{{company_business_unit_uuid}}?include=companies,company-business-unit-addresses` request. Make sure that the response contains:
  - Only the company business unit resource that your current company user belongs to. 
  - `companies` and `addresses` relationships.
- Send the `GET http://glue.mysprykershop.com/company-business-unit-addresses/{{company_business_unit_address_uuid}}` request. Make sure that response contains only the company business unit address of the business unit your current company user belongs to has.
...
- Send the `GET http://glue.mysprykershop.com/company-roles/{{company_role_uuid}}?include=companies` request. Make sure that the response contains:
...
- Send the `GET http://glue.mysprykershop.com/company-users/mine?include=company-roles,companies,company-business-units,customers` request. Make sure that the response contains:
    ...
    - The `company-roles`, `companies`, `company-business-units`, and `customers` relationships.
- Send the `GET http://glue.mysprykershop.com/company-users/{{company_user_uuid}}?include=company-roles,companies,company-business-units,customers` request. Make sure that the response contains:
    - One company user
    - The `company-roles`, `companies`, `company-business-units`, and `customers` relationships
:::
