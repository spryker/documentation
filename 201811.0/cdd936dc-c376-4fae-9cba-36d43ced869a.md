...
@(Info)(Verification)(Make sure that the following changes in transfer objects are present:	)
...
@(Info)()(Make a request to `http://example.org/abstract-products/{{abstract_sku}}?include=abstract-product-availabilities`. Make sure that the response includes relationships to the `abstract-product-availabilities` resource.)
@(Info)()(Make a request to `http://example.org/concrete-products/{{concrete_sku}}?include=concrete-product-availabilities`. Make sure that the response includes relationships to the `concrete-product-availabilities` resource.)
