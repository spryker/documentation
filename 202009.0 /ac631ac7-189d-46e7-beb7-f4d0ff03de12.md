...
After the sales representative has processed the request, the buyer gets it back with the status **Ready**. At this point, if buyers are satisfied with such a request, they can convert the RFQ to the cart, and after that, proceed to the checkout. When the order is placed, the RFQ receives the **Closed** status.
...
To create an RFQ on behalf of a user, see [Creating an RFQ for a Customer](https://documentation.spryker.com/docs/managing-rfqs-sales-rep-shop-guide#creating-an-rfq-for-a-customer)
