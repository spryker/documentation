...
In the ZED, when an administrator is assigning customers to users, a XSS attack (with user interaction) is possible.
...
```
* Check `AbstractAssignmentTable::getSelectCheckboxColumn()` and `AbstractCustomerTable::getCheckboxColumn()` if they were changed on project level.

## Stored Cross-Site-Scripting (XSS) in product reviews 

It is possible for an attacker to enter JavaScript or HTML code which will be executed in ZED.
...
## Open redirect in login script of administrative backend (ZED)
It is possible to manipulate the parameter "referer" and redirect the administrator to any URL.
The login script of the ZED backend allows the specification of a path named "referer". This parameter allows it to redirect an unauthenticated administrator to a specific page after his login.
...
There is a possibility to set an incorrect parameter in the ZED backend on the *edit CMS redirect* page that will redirect an user to another site. By default, those redirects should only be possible to the same application.
...
In the ZED administrative backend, the written product reviews are presented (/product-review-gui). At this point, the administrator can decide if a product review will be approved or rejected. The actions for this do not have any CSRF-Token implemented, due to that a simple click will reject or approve a product review.

**Changes**:
* Adjusted `ProductReviewTable` to generate approve/reject buttons with the CSRF protection form.

**BC breaking impact**:
* `UpdateController::approveAction()` now becomes POST only and must be called by `CloseReclamationForm` submit in order to use CSRF protection.
* `UpdateController::rejectAction()` now becomes POST only and must be called by `CloseReclamationForm` submit in order to use CSRF protection.
...
**Changes**:
* `Adjusted ReclamationTable::createCloseAction()` to generate buttons with CSRF protection form.

**BC breaking impact**:
* Adjusted `DetailController::closeAction()` in order to use CSRF protection, it can handle POST requests only and must be called by `CloseReclamationForm` submit.
...
There is a difference in frontend (Yves) and backend (Zed) password policy in the next functionality:
* Registration
* Change password
* Forgot password
...
* Introduced `NotCompromisedPassword` validation, which will allow users to use only safe passwords.
...
All site visitors can create their own cookies with a chosen name, and this cookie name can be a big value (up to 4Kb). That we will store to Redis, and it can waste server memory.

**Changes**:
* Adjusted `SessionKeyBuilder::buildSessionKey()` to check maximum session key length. By default, the session key length is 32 symbols. We will trim all keys that are longer than 64 symbols.
