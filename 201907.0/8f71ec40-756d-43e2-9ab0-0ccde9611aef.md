...
@(Warning)(Verification)(Once you have finished the full integration of the feature, make sure that:</br>You can store and retrieve data from the vault using `VaultFacade`:</br>`$secret = "actual_secret";` </br>`$vaultFacade->store("secret_category", "secret_id", $secret);`</br>`assertSame($secret, $vaultFacade->retrieve("secret_category", "secret_id"));`)
