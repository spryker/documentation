...
@(Warning)(Verification)(Make sure that the following change has been applied by checking your database:<table><thead><tr><th>Database Entity</th><th>Type</th><th>Event</th></tr></thead><tbody><tr><td>`spy_sales_order_threshold`</td><td>table</td><td>created</td></tr></tbody></table>)
...
```
 ...
 @(Warning)(Verification)(Make sure that: </br>On quote loading, the quote is expanded with data from `table spy_quote_approval` database table. </br>  Before quote deletion, the records from `spy_quote_approval` database table related to quote is removed.)
