...
@(Warning)(Verification)(After adding items to the cart, the following things must be done: )(</br>1) Quotes shared with the current customer must be added to the multi-cart session.</br>2) `CustomerTransfer::$permissions` must contain permissions and be updated in the customer session.)
...
@(Warning)(Verification)(Make sure that the following plugin has been registered:<ul><li>Open Yves and log in with customer.</li><li>Open https://mysprykershop.com/multi-cart/ - the page should contain all customer's quotes.</li><li>Click on the share link. The share cart page should open.</li></ul>)