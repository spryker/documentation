...
@(Warning)(Verification)(Make the following checks at https://mysprykershop.com/quick-order : `ProductPriceItemValidatorPlugin` is responsible for prices. Provide SKUs with and without Volume **Price on Quick Add To Cart** page and verify if quantity changes in the row result in the correct price display.)
...
@(Warning)(Verification)(Make sure that translations data was added to the `spy_glossary` table in the database.)
