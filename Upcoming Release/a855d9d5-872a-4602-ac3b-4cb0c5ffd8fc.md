...
The token(s) are marked as expired on the date and time of the request. After expiration, tokens can be deleted from the database. For this purpose, you need to set the lifetime for **expired** tokens. For details, see Configuring Outdated Refresh Token Life Time. 
