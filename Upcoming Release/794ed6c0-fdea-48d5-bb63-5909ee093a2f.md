...
To create a guest cart for an unauthenticated user, place one or more item on a guest cart by sending the request:
...
#### Request Header
Use the `X-Anonymous-Customer-Unique-Id`header key to specify a unique guest user ID.

| Header key | Header value | Mandatory | Description | 
|---|---|---|---|
| X-Anonymous-Customer-Unique-Id | Alpha-numeric value. | ✓ | Unique guest user ID. | 

:::(Info) (Existing guest users)
If a guest user with the provided ID exists, the items are added to their existing cart instead of creating a new one.
:::
