...
Before you deep-dive into the Spryker update process, you need to understand the Spryker product structure so you could decide what exactly and why you need to update. 
...
The set of features make up the **product**. Spryker offers the following products:

* B2B Demo Shop
* B2C Demo Shop
* Master Suite
See [ Demo Shops](https://documentation.spryker.com/docs/demoshops) for detailed information on each of the products.
...
## Planning the Update
This section provides recommendations on how you can build a process around upgrading your Spryker project so you can plan, estimate, and continuously deliver just like any other feature you are building.
...
1. Go to `https://github.com/[bundle-name-here]/compare/[your-version]…[available-version]`.  For example [https://github.com/spryker/queue/compare/1.1.2...1.2.0](https://github.com/spryker/queue/compare/1.1.2...1.2.0))
...
```
If there are no extra dependencies found, *composer.json* will be updated, respectively. Otherwise, see the [Troubleshooting](https://documentation.spryker.com/docs/updating-a-spryker-based-project#troubleshooting) section at the end of the article, *In case when update is not possible* part.
...
3.  Check for project changes, just like for the *minor* updates in the section above. Go to  `https://github.com/[bundle-name-here]/compare/[your-version]…[available-version]` and check if there are any changes that might conflict with your business logic.

## Test After Each Iteration
Once you've completed the update, it's necessary to make sure that everything still works as expected, and nothing is broken. To do so, complete the following steps right after the upgrade:

### 1. Run automative tests
Automotive tests are must-have for every project in general and are very helpful in case of updates. We recommend to have the following covered:
...
We find the following static code analysis tools the most helpful for running after the upgrade and strongly recommend that you use them:
...
* [Architecture Sniffer](https://github.com/spryker/architecture-sniffer) - helps you to maintain the quality of the architecture. See [Architecture Sniffer](https://documentation.spryker.com/docs/architecture-sniffer) for information on how to run the tool.
...
* **Use a composition pattern**. There is a lot of information on the internet for *composition vs. inheritance* that will help you to understand the pattern. The main idea is to introduce a new class at a project level and to use the class you wanted to inherit as a constructor dependency, i.e. kind of "wrapping". Even though this is a very good practice, it might not work in cases when you want to change protected methods.
...
To exclude / include sniff rules, find your `ruleset.xml` file and adjust the following section:
...
* **Contribute to documentation**: if you found an issue in a migration guide or some parts of it are missing - you can suggest a change by clicking "Edit or Report" button right below the article title.