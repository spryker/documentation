...
## Mac OSX: Installation fails or project folder con not be mounted due to SIP
...
This could have the effect that even though the VM is correctly installed and started, the `/project` folder containg the application to be run, can not be mounted inside the VM, which means that the whole application can not be startted und run. If logged into the VM via `vagrant ssh`a not properly mounted project folder can be identified by running the `ls` command which will return the following error message:

```bash
ls: cannot open directory '.': Stale file handle
```

To solve this problem, you need to disable SIP during the installation:
...
Now SIP is disabled and you can start your installation. After it’s finished, it is strongly recommended enabling SIP again.
You can do this by following the previous steps again with typing `csrutil enable` at Step 4. To determine, whether SIP is enabled or not, you can type `csrutil status`.

@(Warning)()(However, it is possible that the enabling SIP again will prevent the VM from mounting the `/project folder`. In this case please contact devops if there's a solution or if permanent disabling of SIP is an acceptable trade off.)
