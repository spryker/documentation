...
In the previous version of PriceProduct module, a product had only one price per store (and multi-currency) with gross and net amounts.
...
To save BC, we have implemented Default Price Dimension inside the new PriceProduct module, so all prices imported by new PriceProductDataImport will be in the Default Price Dimension. See Prices per Merchant Relation to learn more about the price dimension.
