...
@(Error)()(Care should be taken in project level code to handle the availability and stock values as Decimal objects. Due to the PHP floating-point precision issues, all Decimal numbers in Spryker are wrapped in a `decimal-object` that handles the calculations performed on them to allow exact precision. Read more about the [`decimal-object` library](https://documentation.spryker.com/docs/ht-integrate-and-use-precise-decimal-numbers).) 
