...
| `opcache.memory_consumption`    | To avoid an automatic reset of the Opcache, these values must be high enough. You can look into the PHP info (e.g., in Zed browse to `/maintenance/php-info`) to see the current usage. You can count the number of classes in your code base to get an idea of a good value. | ?          |             |
...
For example, `{{ data.foo.bar.firstName }}` `{{ data.foo.bar.lastName }}` trigger many calls to the `Template::getAttribute()` method which is very slow. Making calculations on the PHP side can help here a lot, as well as using `{{ set customer = data.foo.bar }}` + `{{ customer.firstName }}` `{{ customer.lastName }}`. 
...
Check if you need the `can` method calls from Twig. For example, `{% if can('SeePricePermissionPlugin') %}`. Talking back from Twig to PHP is often slow, so try to avoid that by checking if you need all used Twig functions Spryker provides.
