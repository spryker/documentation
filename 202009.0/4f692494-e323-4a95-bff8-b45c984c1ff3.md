...
| --- | --- | --- | --- |
| username | string | v | Customer's username. You define it when [creating a customer](. |
| password | password | v | Customer's password. You define it when [creating a customer](. |
...
```json
{
    "data": {
        ...
        "links": {
            "self": "http://glue.de.spryker.local/access-tokens"
        }
    }
}
```
...
<a name="customer-authentication-attributes"></a>
...
| accessToken | String | Authorization token used to send requests to the protected resources available for this agent assist.  |
| refreshToken | String | Authorization token used to refresh the `accessToken`. |
...
* [Managing customer authentication tokens](https://documentation.spryker.com/docs/managing-customer-authentication-tokens)