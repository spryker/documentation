...
2. Enable Xdebug by updating `/etc/php/7.2/mods-available/xdebug.ini` with the following:
...
Define servers in PhpStorm:
    ...
    2. Add a new server:
        * Select **+**.
        * Enter a **Name**. For example, `zed.mysprykershop.com` (it is used in CLI below).
        * For **Host**, enter `~^zed\.de\..+\.local$`.
        * Select **Use path mappings**.
        * For **Absolute path on server**, select `/data/shop/development/current`.
