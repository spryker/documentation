...
Since its version `3.17.0` `spryker/zed-request` module provides the ability to collect data about all the **requests to Zed** sent from Yves. To be able to do that one needs to add `\Spryker\Yves\ZedRequest\Plugin\WebProfiler\WebProfilerZedRequestDataCollectorPlugin` to the stack returned by `\Pyz\Yves\WebProfilerWidget\WebProfilerWidgetDependencyProvider::getDataCollectorPlugins()`.
If you are using Redis as a storage, you are also able to track the calls to it. Starting from version `2.4.0` new data collector plugin `\Spryker\Yves\Redis\Plugin\WebProfiler\WebProfilerRedisDataCollectorPlugin` is available for that matter.
Same goes for Elasticsearch. Make sure you use version `1.8.0` of `spryker/search-elasticsearch` and just use `\Spryker\Yves\SearchElasticsearch\Plugin\WebProfiler\WebProfilerElasticsearchDataCollectorPlugin`.
