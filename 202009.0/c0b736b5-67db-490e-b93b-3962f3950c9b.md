...
@(Info)()(The `QueryInterface` instance is a stateful class; sometimes `getSearchQuery()` method is called multiple times and alters the original query (see [Expandig queries](#expanding)), so you need to make sure that it returns the same instance. This can be achieved by creating the `\Elastica\Query` instance atconstruction time and returning it in the `getSearchQuery()` method.)
