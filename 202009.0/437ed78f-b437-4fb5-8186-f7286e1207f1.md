...
## Refresh an agent assist access token



To refresh an agent assist access token, send the request:
...
| --- | --- | --- | --- |
| refreshToken | String | v | The authentication token used to refresh `accessToken`. You can get it by [authenticating as an agent assist](https://documentation.spryker.com/docs/authenticating-as-an-agent-assist#authenticate-as-an-agent-assist) . |
...
| expiresIn | Integer | The time in seconds in which the `accessToken` token expires. |
...
To revoke an agent assist access token, send the request:
...
| --- | --- |
| ***{{refresh_token}}*** | Defines the refresh token to revoke. Enter `mine` to revoke all the refresh tokens of the authenticated agent assist. |
...
| --- | --- | --- | --- |
| X-Agent-Authorization | string | v | Only required when revoking all the refresh token of an agent assist. A string containing digits, letters, and symbols that authorize the agent assist. [Authenticate as an agent assist](https://documentation.spryker.com/docs/authenticating-as-an-agent-assist#authenticate-as-an-agent-assist) to get the value.  |
