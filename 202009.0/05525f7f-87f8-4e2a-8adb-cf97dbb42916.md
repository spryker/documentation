...
* `groups: applications: application: http: max-request-body-size:` - defines the maximum allowed size of the request body sent to the application, in MB. 
  * This parameter can be used to avoid `413 Request Entity Too Large` error during file uploads or requests with big payload.
  * Format: "1m", "0".
  * Default value: zed - `10m`, glue - `2m`, yves - `1m`.
  * Setting size to "0" disables checking of request body size. Not recommended.
