...
In Spryker we are supporting PostgreSQL, MySQL and MariaDB, all are very powerful and come with very promising and handy features. When we are working with databases, it's always good to know these features, one of the very cool features that we like is [CTE](https://www.postgresql.org/docs/9.1/queries-with.html), If you think you are inserting or updating very big data like millions of millions, use CTE as a replaceable for multiple inserts/update. you can find some of Spryker implementation in our demo shops.
...
* Use CTE technique (supported by PostgreSQL, MySQL >= 8 and MariaDB >= 10.2)
