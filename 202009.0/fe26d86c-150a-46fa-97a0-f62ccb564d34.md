...
**Scenario**: We need to route requests made on `URL /hello/{name}` to the action `helloAction(Request $request)` implemented in the `DemoController` which will generate different content based on the value of `name` parameter.
...
You can now access `http://mysprykershop.com/hello/spryker` in your browser to make a request using the newly configured route with parameter `name` having it's value set to `spryker`.
