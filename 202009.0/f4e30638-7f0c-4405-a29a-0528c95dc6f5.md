...
You can stop the VM directly from GUI as well:

1. Open **Oracle VM VirtualBox Manager**

2. Select your virtual machine which is **Running** now

3. Click on `Machine` > `Close` > `Power Off` and your virtual machine will be **Powered Off**

4. Start the virtual machine again with *vagrant up*.
...
Spryker Virtual Machine creates a dynamically allocated storage that grows over time as you add data. However, if you delete the data from the virtual machine later, you'll notice that the disk doesn't automatically shrink. To solve this issue, you may use the [dd](https://en.wikipedia.org/wiki/Dd_(Unix)) utility. To do this, run the following commands inside the folder you placed the source code (by default, it's *devvm*):
