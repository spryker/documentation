To be able to use the FE CSS Lazy Load feature, do the following steps.
1. Update the `ShopUi` module to 1.44.0 version, and also need to update `spryker-shop/shop-dependencie`s into the `CatalogPage`, `HomePage` and `ProductDetailPage` modules, run the following command for this:
```bash
...
```
2. Add `"@jsdevtools/file-path-filter": "~3.0.2"`, into the `package.json` file to the `devDependencies` section and run `npm install`.
...
* Update `findEntryPoints` function using the `mergeEntryPoints` as described above: 
```js
...
// find entry points
...
* Add `findStyleEntryPoints` function:
 ```js
...
...
* Pass the `findStyleEntryPoints` function to the exported module:
```js
...
module.exports = {
...
* Add the `filePathFilter` to the imported stuff at the top of the file:
```js
const filePathFilter = require("@jsdevtools/file-path-filter");
```

* Add the `findStyleEntryPoints` to the import from finder module:
```js
...
...
* Add new local variable `styleEntryPointsPromise` to the `getConfiguration` function:
```js
...
...
* Extend the destructuring assignment with the following changes:
From:
```js
...
* Add new local variables `criticalEntryPoints` and `nonCriticalEntryPoints` to the `getConfiguration` function:
```js
...
...
* Extend `entry` section of the returned Webpack config object into the `getConfiguration` function with the new `critical`, `non-critical`, and `util` points:
```js
...
entry: {
    'vendor': vendorTs,
...
...
```
6. Update the `src/Pyz/Yves/EventDispatcher/EventDispatcherDependencyProvider.php` on project level: 

* Add `LastVisitCookieEventDispatcherPlugin` to using section:
```js
...
...
* Add this plugin to returned collection of the `getEventDispatcherPlugins` function:
```js
...
7. Update the `page-blank.twig` layout on the project level `src/Pyz/Yves/ShopUi/Theme/default/templates/page-blank/page-blank.twig`:

* Add new `isCssLazyLoadSupported` twig variable: 
```html
...
```
:::
Run `npm run yves` to rebuild the frontend with the new settings.