...
In case your DB changes include any operations with data, that's already exists,
Spryker persistence provides functionallity to execute any custom business logic that can be written in PHP
while applying a migration.

To leverage this mechanism one can create a migration file (eg PropelMigration_1622797441.php) 
to the migration folder. At the end of the file name timestamp that's older than the previous migration file timestamp
should be used. Any business logic that impacts your data could be placed in this file for exection.

This migration will be applied only once and registered in the propel_migration table.
