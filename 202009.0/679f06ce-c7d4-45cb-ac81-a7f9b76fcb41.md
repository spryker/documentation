The Product Sets feature allows you to put together multiple products еo emphasize that the set of products can be bought together. Product Sets usually have their separate list and detail pages in the shop frontend where customers can add containing products to the cart.
...
```
In case you want to enable Product Sets search powered by Elasticsearch, you also need to install `spryker/search-elasticsearch` module:
```bash
...
@(Warning)(Sorting Product Sets)(There's a Zed UI that allows you to sort Product Sets easily compared to each other under the *Reorder Product Sets*.
