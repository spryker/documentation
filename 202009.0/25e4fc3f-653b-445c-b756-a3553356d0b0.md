...
Elasticsearch 7, among other changes, continues with the process of removal of mapping types started in version 6. While previous major version has deprecated the concept of mapping types themselves but still required one mapping type per index, Elasticsearch 7.x by default does not allow mapping types at all.

**General considerations**

To communicate with Elasticsearch Spryker uses a 3d party library called `ruflin/elastica`. To be able to interact with Elaticsearch 7.x, `ruffling/elastica:7.*.*` must be used (major version of this package so far refer to major versions of Elasticsearch itself). This version has some drastic changes in its API compared to the previous major. Among them are:
- removal of several query type classes, e.g. `\Elastica\Query\Type` and `\Elastica\Query\GeohashCell`
...
For the full list of changes please check this document https://elastica.io/2019/10/31/release-7-dot-0-0-beta1/. All the project code that's not compatible with these changes must be adjusted accordingly before running Elasticsearch 7.

**Migrating from Elasticsearch 6.x to Elasticsearch 7.x**

Update the modules in use by running the command:
```Bash
...
You won't need to do any extra-work related to your indexes and the data stored. It means, that you don't need to adjust your indexes in any way, e.g. by removing their corresponding mapping types and re-index the data, because both Elasicsearch 7.x and Spryker can work with your indexes created for version 6.x out of the box. But keep in mind, that all the new indexes created in Elasticsearch 7.x must not contain mapping types.

**Migrating from Elasticsearch 5.x to Elasticsearch 7.x**

To be able to migrate from Elasticsearch 5.x to Elasticsearch 7.x first you must follow Search migration concept to make your project code Elasticsearch 6 compatible. Then just follow the guidelines for migrating from Elasticsearch 6.x to Elasticsearch 7.x described above. Be advised that to perform the upgrade without shutting down the cluster you will have to do a rolling upgrade as described here
