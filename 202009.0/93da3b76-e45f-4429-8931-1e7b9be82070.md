...
## Installation
To enable validation of REST requests, install the `RestRequestValidation` module by following [Request Validation Integration](https://documentation.spryker.com/v4/docs/glue-api-rest-schema-validation-feature-integration). 
...
## Validating APIs
To apply validation rules to an APIs:
...
By default, all the fields in a request are required. To make a field optional, write a schema as follows:
...
All validation rules in Spryker Glue API are cached. To apply new or updated rules, you need to re-build the validation cache by running the command: 
