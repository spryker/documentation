...
2. Data in a queue is consumed and imported into a persistent storage. If you already have data in the queues, skip this part and check [Queue data importer tutorial](https://documentation.spryker.com/docs/queue-data-importer-tutorial#queue-data-importer-tutorial). 

## Exporing data from CSV to queue 

If your data is already provided in CSV format, it can be imported via the new Spryker Dataimport feature to the queues.
...
1. Queue name—the name of the resource-based queue which will store the imported data between the steps (i.e., `import.product_abstract`).
...
Specify the name for the method, which will create an instance of `DataImportQueueWriterConfigurationTransfer` and initialize it with the main queue name and the size of a chunk data will be written in.

Next, a plugin class, which will configure and hook up queue writer with the data importer, needs to be created. This plugin must extend `Spryker\Zed\DataImport\Communication\Plugin\AbstractQueueWriterPlugin` and has to provide implementations for its two abstract methods:
...
:::(Warning) (Note)
We not recommend using the QUEUE_READERS group. This executes all the configured queue importers during one import run. Because in a lot of scenarios the order, in which the data is imported, matters and because of the possibility of the race condition, this can lead to various malfunctions/inconsistencies in the imported data. We recommend structuring the import process in a way that would allow importing data via the message queues apart from other imported resources.
:::
