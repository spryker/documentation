The `ContainerInterface` provides a way to make dependencies globally available. Every dependency added to the `ContainerInterface` that is marked as `isGlobal` is available by using `getProvidedDependency()` in your factory.
